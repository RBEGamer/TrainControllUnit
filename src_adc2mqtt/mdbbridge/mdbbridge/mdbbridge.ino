...
void loop() {
  ...
  if (slider_currentMillis - slider_previousMillis >= slider_interval) {
    ...
    if (set_break_level <= 100 && set_break_level >= 0) {
      ...
      if (set_break_level_conv > 80) {
        set_break_level_conv = 4;
      }
      else if (set_break_level_conv > 50) {
        set_break_level_conv = 3;
      }
      else if (set_break_level_conv > 30) {
        set_break_level_conv = 2;
      }
      else if (set_break_level_conv > 15) {
        set_break_level_conv = 1;
      }
      else if (set_break_level_conv <= 15) {
        set_break_level_conv = 0;
      }
    ...
    if (set_velocity_level <= 100 && set_velocity_level >= 0) {
      ...
      //HEARTBEAT
      String tmp = "{\"event\":\"heartbeat\"}";
      int hb_len = tmp.length() + 1;
      char hb_array[hb_len];
      tmp.toCharArray(hb_array, hb_len);
      mqtt_client.publish(MQTT_TOPIC_SEND_EVENT, hb_array);
    }
